...
export const parseTraceExp: (params: Sexp[]) => Result<TraceExp> = 
    (params) =>  params.length !== 1 ? makeFailure("Expression not of the form (trace <varRef>)") :
    parseTraceParams(params[0]);

const parseTraceParams : (param : Sexp) => Result<TraceExp> = 
    (param) => creatTraceResult(parseL4Exp(param));

const creatTraceResult : (exp : Result<Exp>) => Result<TraceExp> = 
    (exp) => isOk(exp) ? isVarRef(exp.value) ? makeOk(makeTraceExp(exp.value)) : makeFailure("not a valid varRef") : exp ;
