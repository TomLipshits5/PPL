...
import { Result, bind, mapv, mapResult, makeFailure, makeOk,isOk, isFailure } from "../shared/result";
...
const evalTraceExp = (exp: TraceExp): Result<void> =>
    checkFb(applyEnvBdg(theGlobalEnv,exp.var.var));

const checkFb = (fb : Result<FBinding>): Result<void> =>
    isOk(fb) ? isClosure(unbox(fb.value.val)) ? resetBinding(fb.value, unbox(fb.value.val)): makeFailure("not a closure"): fb;

const resetBinding = (fb : FBinding, cl : Value): Result<void> =>
    isClosure(cl) ? makeOk(setBox(fb.val,makeTracedClosure(cl,fb.var))): makeFailure("not a closure");
...
const applyTracedClosure = (proc: TracedClosure, args: Value[]): Result<Value> => {
    ...
    proc.count ++ ;
    const vars = map((v: VarDecl) => v.var, proc.params);
    const vals = evalSequence(proc.body,makeExtEnv(vars,args,proc.env));
    if (isOk(vals)) {
        proc.count--;
        printPostTrace(vals.value,proc.count);
